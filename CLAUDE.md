...
- **Code経路のBedrockルーティングは保証されない**: `code-agent/claude-code.js` の `spawn()` は `env` を上書きせず、このNode.jsプロセスの環境変数をそのまま `claude` CLIに継承させるだけ。`CLAUDE_CODE_USE_BEDROCK` はこのアプリのどこにも設定していないため、CLIが実際にBedrock経由で動くかAnthropicに直接繋がるかは完全にCLI自身の設定次第（このアプリからは制御も保証もできない）。`/api/config` の `codeUsesBedrock`（`process.env.CLAUDE_CODE_USE_BEDROCK` が truthy かどうかだけを見る）を見て、未設定ならCodeタブに切り替えた瞬間にクライアント側で警告バナーを出す（`setSidebarTab` 内、`showSetupBanner` 経由）。ハードブロックはしない設計——CLIの `settings.json` 側でBedrock経由に設定されている場合など、環境変数以外の手段で正しく設定されているケースを誤って止めないため。
