...
Windows環境での起動は `start.ps1` を使う（`.\start.ps1`。PowerShellスクリプトなので `python .\start.ps1` のように別のインタプリタで実行すると構文エラーになる）。初回起動時にリージョン・プロファイル・プロキシ・CA証明書・ポートを対話で聞き、`start.local.json`（Git管理外）に保存して次回以降はそれを読む。設定をやり直すときは `.\start.ps1 -Reconfigure`。この方式にしたのは、環境固有の値（社内プロキシURLや個人のホームディレクトリのパス）を公開リポジトリのスクリプトに書かないため。以下は対話では聞かない任意項目で、`start.local.json` に直接書く:
