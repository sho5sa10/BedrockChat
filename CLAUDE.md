...
- `ALLOW_ANTHROPIC_DIRECT` — `"1"` でCodeタブの `claude` CLI に対するBedrock経由の強制を解除する。既定は解除しない。詳細は下の「Code経路のBedrockルーティングは強制する」を参照
...
- **Code経路のBedrockルーティングは強制する**（v1.6.4。それ以前は「保証されない」だった）: `code-agent/claude-code.js` の `buildChildEnv()` が、`spawn()` に渡す `env` に `CLAUDE_CODE_USE_BEDROCK=1` と `CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC=1` を立てる。**これはこのアプリのコンセプトそのもの**——READMEが掲げる「通信先はBedrockだけ」は、以前は環境の設定次第に委ねられた願望でしかなく、未設定のマシンではAnthropicに直接繋がり、Bedrock経由に設定済みでも自動アップデート・テレメトリ・エラーレポートはBedrock以外へ出ていた。**`env` を渡さない実装（＝環境の丸ごと継承）に戻さないこと。** 変数名は2つともインストール済みCLIバイナリ内の文字列を確認済み。
  - **強制は絶対ではない**: CLIは `settings.json` の `env` を継承環境変数の**上に**適用するので、そこで明示的に `"0"` にされていればそちらが勝つ。これは正しい挙動（明示的なポリシーがアプリの既定に勝つべき）で、その場合は下記の判定が `source:"settings"` として断定的に報告する。強制が塞ぐのは「何も設定していないマシンが黙ってAnthropicに行く」という一番多いケース。
  - `ALLOW_ANTHROPIC_DIRECT=1` で強制を解除できる（`ALLOW_CROSS_REGION_INFERENCE` と同じ形。`start.local.json` に書く任意項目で対話では聞かない）。解除時は非必須通信の停止も外す——意図的にAnthropic直で使う人に、こちらのロックダウンを押し付ける理由がないため。`resolveBedrockRouting()` もこのフラグを見る（見ないと、意図的にAnthropicへ行かせた実行に対して「Bedrock経由」と表示してしまう）。
  - 判定は `code-agent/cli-config.js` の `resolveBedrockRouting()` に集約し、`/api/config` が毎回解決して `codeUsesBedrock` / `codeBedrockSource` / `codeBedrockDetail` を返す。確認できなかったときだけ、Codeタブに切り替えた瞬間にクライアント側で警告バナーを出す（`setSidebarTab` 内、`showSetupBanner` 経由）。強制があるので `source:null`（＝確認できない）に到達するのは**解除された場合だけ**になり、バナーの文言もそれ前提で書いてある。
  ...
  - ハードブロックはしない設計。ラッパースクリプト、起動ごとのフラグ、Windowsのレジストリ経由のポリシーなど、ここから見えない設定手段が残るため。設定ファイルで明示的に無効化されている場合（`source:"settings"`）と、強制を解除した場合（`source:null`）で文言を分けている。
